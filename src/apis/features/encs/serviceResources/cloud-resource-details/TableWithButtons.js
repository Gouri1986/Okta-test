...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-resource-master`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (cloudResourceMasterId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-resource-master`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { cloudResourceMasterId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(cloudResourceMasterId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
  ...
  const columns = [
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.cloudResourceName}</span>
          </div>
        </div>
      )
    },
    {
      name: 'Description',
      selector: 'cloudResourceMasterDescription',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '150px' }}>{row.cloudResourceMasterDescription}</span>
          </div>
        </div>
      )
    },
    {
      name: 'cloud Service Master',
      selector: 'cloudServiceMasterId',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '150px' }}>{row.cloudServiceName}</span>
          </div>
        </div>
      )
    },
    {
      name: 'Cloud Dependent Services',
      selector: 'cloudDependentServices',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '150px' }}>{row.cloudDependentServices}</span>
          </div>
        </div>
      )
    },
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate ml-1'>
            <span className='d-block font-weight-bold text-truncate'>{row.encsTags}</span>
          </div>
        </div>
      )
    },
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.cloudResourceMasterId)}>
              <Trash size={15} />
            </div>
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = data.filter((item) => {
        const startsWith =
          item.cloudResourceName.toLowerCase().startsWith(value.toLowerCase()) ||
          item.cloudResourceMasterDescription.toLowerCase().startsWith(value.toLowerCase())

        const includes =
          item.cloudResourceName.toLowerCase().includes(value.toLowerCase()) ||
          item.cloudResourceMasterDescription.toLowerCase().includes(value.toLowerCase())
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='Cloud Resource Master'
        breadCrumbParent='Services & Resources'
        breadCrumbActive='Cloud Resource Master'
      />
