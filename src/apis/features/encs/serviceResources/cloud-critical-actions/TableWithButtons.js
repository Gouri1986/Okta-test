...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-critical-actions`,
      {
        headers: {
      ...
      .then((response) => {
        ...
        //  setIsLoaded(true)
        console.log("cloud-critical-actions", response.data)
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (cloudCriticalActionId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-critical-actions`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { cloudCriticalActionId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(cloudCriticalActionId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
  ...
  const columns = [
    ...
    {
      name: 'Cloud Critical Action Title',
      selector: 'cloudCriticalActionTitle',
      sortable: true,
      minWidth: '350px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '320px' }}>{row.cloudCriticalActionTitle}</span>
    ...
    {
      name: 'Description',
      selector: 'cloudCriticalActionDescription',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '250px' }}>{row.cloudCriticalActionDescription}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '120px' }}>{row.cloudType}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '120px' }}>{row.cloudName}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '220px' }}>{row.cloudReferenceResources?.cloudResourceName}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '220px' }}>{row.cloudReferenceResources?.cloudServiceName}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.cloudCriticalActionId)}>
              <Trash size={15} />
            </div>
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='Cloud Critical Action'
        breadCrumbParent='Services & Resources'
        breadCrumbActive='Cloud Critical Action'
      />
