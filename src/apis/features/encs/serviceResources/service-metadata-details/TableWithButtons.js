...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-service-metadata-details`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (cloudServiceMetadataName) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-service-metadata-details`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { cloudServiceMetadataName: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(cloudServiceMetadataName, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '250px' }}>{row.cloudServiceMetadataDescription}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      selector: 'cloudServiceType',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.cloudServiceType}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.cloudServiceMetadataName)}>
              <Trash size={15} />
            </div>
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = data.filter((item) => {
        const startsWith =
          item.cloudServiceMetadataDescription.toLowerCase().startsWith(value.toLowerCase()) ||
          item.cloudServiceMetadataName.toLowerCase().startsWith(value.toLowerCase())
        item.cloudServiceType.toLowerCase().startsWith(value.toLowerCase())

        const includes =
          item.cloudServiceMetadataDescription.toLowerCase().includes(value.toLowerCase()) ||
          item.cloudServiceMetadataName.toLowerCase().includes(value.toLowerCase())
        item.cloudServiceType.toLowerCase().startsWith(value.toLowerCase())
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='Cloud Service Meta Data Details'
        breadCrumbParent='Services & Resources'
        breadCrumbActive='Cloud Service Meta Data Details'
      />
