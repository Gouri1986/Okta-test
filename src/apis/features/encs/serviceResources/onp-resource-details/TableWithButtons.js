...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-onp-resource-master-details`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (onpResourceMasterId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-onp-resource-master-details`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { onpResourceMasterId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(onpResourceMasterId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
  ...
  const columns = [
    ...
    {
      name: 'ONP Resource Name',
      selector: 'onpResourceName',
      sortable: true,
      minWidth: '290px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.onpResourceName}</span>
    ...
    {
      name: 'Description',
      selector: 'onpResourceMasterDescription',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '150px' }}>{row.onpResourceMasterDescription}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '150px' }}>{row.onpServiceName}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.onpResourceMasterId)}>
              <Trash size={15} />
            </div>
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = data.filter((item) => {
        const startsWith =
          item.onpResourceName.toLowerCase().startsWith(value.toLowerCase()) ||
          item.onpResourceMasterDescription.toLowerCase().startsWith(value.toLowerCase())

        const includes =
          item.onpResourceName.toLowerCase().includes(value.toLowerCase()) ||
          item.onpResourceMasterDescription.toLowerCase().includes(value.toLowerCase())
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='ONP Resource Master'
        breadCrumbParent='Services & Resources'
        breadCrumbActive='ONP Resource Master'
      />
