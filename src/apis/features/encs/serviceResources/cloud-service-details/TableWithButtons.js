...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-service-master`,
      {
        headers: {
  ...
  const removeData = (cloudSerivceMasterId) => {
    const DeletePassFunction = (data) => {
      ...
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-cloud-service-master`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { cloudSerivceMasterId: deletevar }
        }
      ).then((response) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: 'cloudServiceName',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.cloudServiceName}</span>
    ...
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '250px' }}>{row.cloudServiceMasterDescription}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    ...
    },
    {
      ...
      selector: 'cloudServiceGroup',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '230px' }}>{row.cloudServiceGroup}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.cloudSerivceMasterId)}>
              <Trash size={15} />
            </div>
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='Cloud Service Master'
        breadCrumbParent='Services & Resources'
        breadCrumbActive='Cloud Service Master'
      />
