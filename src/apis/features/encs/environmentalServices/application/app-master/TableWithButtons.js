...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-app-env-master`,
    {
      headers: {               
...
//--Delete method-
const removeData = (appEnvName, applicationId) => {
  const DeletePassFunction = (data, data2) => {
    const appname_delete = data
    const appid_delete = data2
    console.log("deletevar = ", appid_delete)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-app-env-master`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        ...
        data : {
          appEnvName : appname_delete,
          applicationId : appid_delete
        }
      }
      ).then((response) => {
          ...
          if (response.data.message === "Record deleted successfuly") {
            toast.success(
              <ToastContentSuccess message={response.data.message} deletevar={appname_delete}/>,
              { transition: Slide, hideProgressBar: true, autoClose: 4000 }
            )
          } else {
            toast.error(
              <ToastContentFail message={response.data.message} deletevar={appname_delete}/>,
              { transition: Slide, hideProgressBar: true, autoClose: 4000 }
            )
          }
          fetchData()
  ...
  handleConfirmText(appEnvName, applicationId, {DeletePassFunction}) 
}
 const columns = [
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'  style={{width:'280px'}}>{row.appEnvDescription}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  onClick={() => removeData(row.appEnvName, row.applicationId)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>Application Environment Master</CardTitle>
