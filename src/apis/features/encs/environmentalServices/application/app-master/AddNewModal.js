...
//-Dropdown selection handler End-
 ...
 const [token, setToken] = useState(
  ...
  const handlePost = (evt) => {
    ...
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-app-env-master`, {
...
//--Fetching Dropdown ends
  ...
  const SignupSchema = yup.object().shape({
    ApplicationEnvironment: yup.string().min(5).required(),
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'style={{color:'black'}}>New Application Environment</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1' onSubmit={handleSubmit(onSubmit)}>
      <FormGroup>
          <Label for='ApplicationEnvironment' style={{fontSize:'1rem'}}>Application Environment</Label>
          <InputGroup>
            ...
            <Input id='ApplicationEnvironment'
             name='ApplicationEnvironment' 
             innerRef={register({ required: true })} 
             invalid={errors.ApplicationEnvironment && true} 
             placeholder='' onChange={(e) => setIscOsCategoryName(e.target.value)} />
            {errors && errors.ApplicationEnvironment && <FormFeedback>Application Environment must be at least 5 characters</FormFeedback>}
          </InputGroup>
        </FormGroup>
      ...
      <FormGroup>
          ...
          <InputGroup>
            ...
            <Input type='textarea' rows='3' id='Description' name='Description' innerRef={register({ required: true })} invalid={errors.Description && true} placeholder=''  onChange={(e) => setIscOsCategoryDescription(e.target.value)} />
