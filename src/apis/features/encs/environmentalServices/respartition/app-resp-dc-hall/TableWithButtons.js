...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-app-res-partition-dc-hall`,
    {
      headers: {               
...
//--Delete method-
const removeData = (appResPartitionDcHallId) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-app-res-partition-dc-hall`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { appResPartitionDcHallId : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(appResPartitionDcHallId, {DeletePassFunction}) 
}
//--Delete method End-
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  onClick={() => removeData(row.appResPartitionDcHallId)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>Application Resource Partition DC Hall</CardTitle>
