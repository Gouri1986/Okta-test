...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-dc-asset`,
    {
      headers: {               
  ...
  .then((response) => {
    // Handle success.
    console.log("Connection established.Data is fetching!", response)
   //  setIsLoaded(true)
...
const removeData = (dcAssetId) => {
  const DeletePassFunction = (data) => {
    ...
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-dc-asset`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
 ...
 const columns = [
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate ml-1'>
          <span className='d-block font-weight-bold text-truncate'>{row.dcAssetName}</span>
        </div>
      </div>
    )
  },
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate' style={{width:'250px'}}>{row.dcAssetModelName}</span>
  ...
  },
  {
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate' style={{width:'250px'}}>{row.supplierName}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
  },
  {
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate' style={{width:'250px'}}>{row.customerBusinessName}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
  ...
  //     <div className='d-flex align-items-center'>
  //       <div className='user-info text-truncate '>
  //         <span className='d-block font-weight-bold text-truncate' style={{width:'250px'}}>{row.filenameColumn}</span>
  //         {/* <small>{row.post}</small> */}
  //       </div>
  //     </div>
  //   )
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>DC Asset</CardTitle>
