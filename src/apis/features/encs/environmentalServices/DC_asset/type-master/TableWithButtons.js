...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-dc-asset-type-master`,
    {
      headers: {               
...
//--Delete method-
const removeData = (dcAssetTypeId) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-dc-asset-type-master`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { dcAssetTypeId : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(dcAssetTypeId, {DeletePassFunction}) 
}
//--Delete method End-
 ...
 const columns = [
  ...
  {
    name: 'DC Asset Type Name',
    selector: 'dcAssetTypeName',
    sortable: true,
    minWidth: '250px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate ml-1'>
          <span className='d-block font-weight-bold text-truncate'>{row.dcAssetTypeName}</span>
        </div>
      </div>
    )
  },
  {
    name: 'Description',
    selector: 'dcAssetTypeDesc',
    sortable: true,
    minWidth: '250px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'style={{width:'200px'}}>{row.dcAssetTypeDesc}</span>
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  onClick={() => removeData(row.dcAssetTypeId)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>DC Asset Type Master</CardTitle>
