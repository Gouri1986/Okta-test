...
const AddNewModal = ({ open, handleModal, fetchData }) => {
  ...
  const handlePost = (evt) => {
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-aws-app-res-partition-az`, {
          appSubPartitionId : subPartitionId,
          appSubPartitionName : subPartitionname,
          awsResPartitionAzId :resPartitionAzId
      },
      {
        headers: {
  ...
  useEffect(() => { 
    // if (isLoaded === false) {
      axios.get(`${process.env.REACT_APP_URL}/api/v1/list-awsResPArtitionAzId`
    )
    .then((response) => {
      // Handle success.
      console.log("Connection established.Data is fetching!")
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'style={{color:'black'}}>New AWS Application Resource Partition AZ</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1' onSubmit={handleSubmit(onSubmit)}>
      
        <FormGroup>
        <Label for='Partition-AZ' style={{fontSize:'1rem'}}>AWS App Resource Partition AZ</Label>
        <InputGroup>
        <InputGroupAddon addonType='prepend'>
          ...
          <option selected hidden disabled>Choose App Resource Partition AZ</option>
            {datatwo.map(item => (
              <option
                key={datatwo.awsResPartitionAzId}
                value={datatwo.awsResPartitionAzId}
                
              >
                {item.awsResPartitionAzId}
              </option>
            ))}
        </Input>
        </InputGroup>
        ...
        <FormGroup>
        <Label for='Sub-Partition' style={{fontSize:'1rem'}}>Application Sub Partition</Label>
        <InputGroup>
        <InputGroupAddon addonType='prepend'>
              <InputGroupText>
              <Layers size={15}/>
          ...
          <Input type='select' name='select' id='Sub-Partition' onChange={changepartition}>
          <option selected hidden disabled>Choose Application Sub Partition</option>
            {data.map(item => (
              <option
