...
const UpdateModal = ({ open, handleUpdateModal, value, fetchData}) => {
  ...
  //-------------------------Update Method---------------------------------------------
  useEffect(() => {
    setappResPartitionAzId(value.awsAppResPartitionAzId)
   })
  useEffect(() => {
    setsubPartitionname(value.appSubPartitionName)
    setsubPartitionId(value.appSubPartitionId)
    setresPartitionAzId(value.awsResPartitionAzId)
 }, [value])

  const handlePost = (evt) => {
    axios.put(`${process.env.REACT_APP_URL}/api/v1/encs-aws-app-res-partition-az`, {
      appSubPartitionId : subPartitionId,
      appSubPartitionName : subPartitionname,
      awsAppResPartitionAzId :appResPartitionAzId,
...
return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleUpdateModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'style={{color:'black'}}>Update AWS Application Resource Partition AZ</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1'>
        ...
        <InputGroupAddon addonType='prepend'>
          ...
          <Input type='select' name='select' id='Sub-Partition'
