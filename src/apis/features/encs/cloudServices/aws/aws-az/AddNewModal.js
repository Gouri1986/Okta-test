...
import { X, Plus, Layers, Command, Database } from 'react-feather'
import {
  Button,
  Modal,
  ModalHeader,
...
const AddNewModal = ({ open, handleModal, fetchData }) => {
  ...
  const handlePost = (evt) => {
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-aws-az-details`, {
      azCity: city,
  ...
  const SignupSchema = yup.object().shape({
    ...
    azID: yup.string().min(10).required(),
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title' style={{ color: 'black' }}>New AWS AZ Details</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1' onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input id='azID' name='azID' innerRef={register({ required: true })} invalid={errors.azID && true} placeholder='' onChange={(e) => setidAz(e.target.value)} />
            {errors && errors.azID && <FormFeedback>AZ ID must be at least 10 characters</FormFeedback>}
          </InputGroup>
        </FormGroup>
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input type='select' name='select' id='AZREgion' onChange={ChnageAzRegion}>
              <option hidden selected disabled>Choose AZ Region</option>
              {data.map(item => (
                <option
                  key={data.regionId}
                  value={data.regionId}
