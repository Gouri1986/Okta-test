...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-aws-az-details`,
      {
        headers: {
  ...
  const removeData = (azId, azRegionId) => {
    const DeletePassFunction = (data, data2) => {
      ...
      console.log("deletevar = ", del_azId)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-aws-az-details`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
