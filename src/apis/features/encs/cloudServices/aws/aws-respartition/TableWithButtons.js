...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-aws-res-partition-az`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (awsResPartitionAzId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-aws-res-partition-az`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { awsResPartitionAzId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(awsResPartitionAzId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.awsResPartitionAzId)}>
              <Trash size={15} />
            </div>
