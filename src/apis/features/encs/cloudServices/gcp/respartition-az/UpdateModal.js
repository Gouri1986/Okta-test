...
const UpdateModal = ({ open, handleUpdateModal, value, fetchData}) => {
  ...
  //-------------------------Update Method---------------------------------------------
  useEffect(() => {
    setgcpResPartitionAzId(value.gcpResPartitionAzId)
  ...
  const handlePost = (evt) => {
    axios.put(`${process.env.REACT_APP_URL}/api/v1/encs-gcp-res-partition-az`, {
      gcpResPartitionAzId:gcprespartitionazid,
      azId : idAz,
      azRegionId : regionId,
...
return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleUpdateModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'style={{color:'black'}}>Update GCP Resource Partition AZ</h5>
      </ModalHeader>
      ...
      <FormGroup>
        ...
        <InputGroupAddon addonType='prepend'>
          ...
          <Input type='select' name='select' id='partitionname'
          value={partitionName}
        ...
        <InputGroupAddon addonType='prepend'>
          ...
          <Input type='select' name='select' id='azid'
          value={idAz}
          onChange={ChangeAzid}>
            {datafour.map(item => (
              <option
        ...
        </FormGroup>           <Button className='mr-1' color='primary' 
        onClick={ () => {
          handlePost()
          handleUpdateModal() 
