...
const AddNewModal = ({ open, handleModal, fetchData }) => {
  ...
  const handlePost = (evt) => {
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-gcp-az-details`, {
      azCity: city,
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title' style={{ color: 'black' }}>New GCP AZ Details</h5>
      </ModalHeader>
        ...
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input type='select' name='select' id='AZRegion' onChange={ChnageAzRegion}>
              <option disabled hidden selected>Choose AZ Region</option>
              {data.map(item => (
                <option
                  key={data.regionId}
                  value={data.regionId}
