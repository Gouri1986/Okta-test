...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-gcp-app-res-partition-az`,
      {
        headers: {
  ...
  const removeData = (gcpAppResPartitionAzId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-gcp-app-res-partition-az`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { gcpAppResPartitionAzId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(gcpAppResPartitionAzId, { DeletePassFunction })
  }
  ...
  const columns = [
    ...
    {
      name: 'GCP Resource Partition AZ',
      selector: 'gcpResPartitionAzId',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '200px' }}>{row.gcpResPartitionAzId}</span>
    ...
    },
    {
      ...
      selector: 'appSubPartitionId',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate ml-1'>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '200px' }}>{row.appSubPartitionName}</span>
          </div>
        </div>
      )
    },
    // {
    //   name: 'gcp App ResPartition AZ Id',
    //   selector: 'gcpAppResPartitionAzId',
    //   sortable: true,
    //   minWidth: '290px',
    //   cell: row => (
    //     <div className='d-flex align-items-center'> 
    //       <div className='user-info text-truncate '>
    //         <span className='d-block font-weight-bold text-truncate'style={{width:'200px'}}>{row.gcpAppResPartitionAzId}</span>
    //         {/* <small>{row.post}</small> */}
    //       </div>
    //     </div>
    //   )
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.gcpAppResPartitionAzId)}>
              <Trash size={15} />
            </div>
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      ...
      updatedData = data.filter((item) => {
        try {
          const startsWith =
            item.gcpResPartitionAzId.toLowerCase().startsWith(value.toLowerCase()) ||
            item.appSubPartitionName.toLowerCase().startsWith(value.toLowerCase())

          const includes =
            item.gcpResPartitionAzId.toLowerCase().includes(value.toLowerCase()) ||
            item.appSubPartitionName.toLowerCase().includes(value.toLowerCase())
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='Application Resource Partition AZ'
        breadCrumbParent='GCP'
        breadCrumbActive='Application Resource Partition AZ'
      />
