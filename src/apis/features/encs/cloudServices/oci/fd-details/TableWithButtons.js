...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-oci-fd-details`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (fdId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-oci-fd-details`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
    ...
    handleConfirmText(fdId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate ml-1'>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '200px' }}>{row.fdId}</span>
          </div>
        </div>
      )
    },
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.fdId)}>
              <Trash size={15} />
            </div>
            {/* <Edit size={15} /> */}
          </div>
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='Infrastructure FD Details'
        breadCrumbParent='Oracle Cloud'
        breadCrumbActive='Infrastructure FD Details'
      />
