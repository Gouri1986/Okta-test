...
import { X, Layers, Command, Database, Plus } from 'react-feather'
import {
  Button,
  Modal,
  ModalHeader,
...
const AddNewModal = ({ open, handleModal, fetchData }) => {
  ...
  const handlePost = (evt) => {
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-oci-ad-details`, {
      adId: adid,
      adCity: city,
  ...
  const SignupSchema = yup.object().shape({
    ...
    AdId: yup.string().min(10).required(),
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title' style={{ color: 'black' }}>New OCI AD Details</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1' onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input type='text' id='AdId'
              name='AdId' innerRef={register({ required: true })} invalid={errors.AdId && true} placeholder=''
              onChange={(e) => setadid(e.target.value)} />
            {errors && errors.AdId && <FormFeedback>AD ID must be at least 10 characters</FormFeedback>}
          </InputGroup>
        </FormGroup>
        <FormGroup>
        ...
        <FormGroup>
          <Label for='City' style={{ fontSize: '1rem' }}>AD City</Label>
          <InputGroup>
            <InputGroupAddon addonType='prepend'>
              <InputGroupText>
                <Command size={15} />
        ...
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input type='text' id='Pincode' name='pincode' innerRef={register({ required: true })} invalid={errors.Pincode && true} onChange={(e) => setpincode(e.target.value)} />
