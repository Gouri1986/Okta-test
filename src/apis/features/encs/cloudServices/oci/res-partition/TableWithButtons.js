...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-oci-res-partition-ad`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method----------------------------------------------------------------------
  const removeData = (ociResPartitionAdId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-oci-res-partition-ad`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { ociResPartitionAdId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(ociResPartitionAdId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
  ...
  const columns = [
    ...
    {
      name: 'Resource Partition Name',
      selector: 'resPartitionName',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.resPartitionName}</span>
    ...
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '200px' }}>{row.adId}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '200px' }}>{row.adRegionId}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
...

    {
      ...
      allowOverflow: true,
      cell: row => {
        const id = row.ociResPartitionAdId
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            <div tag='a' className='w-100' id={id}
              onClick={() => {
                handleUpdateModal()
            ...
            <div tag='a' id={id} className='w-100' onClick={() => removeData(row.ociResPartitionAdId)}>
              <Trash size={15} />
            </div>
            {/* <Edit size={15} /> */}
          </div>
  ...
  const handleFilter = e => {
    ...
    if (value.length) {
      updatedData = data.filter(item => {
        try {
          const startsWith =
            item.customerBusinessName.toLowerCase().startsWith(value.toLowerCase()) ||
            item.resPartitionName.toLowerCase().startsWith(value.toLowerCase()) ||
            item.adId.toLowerCase().startsWith(value.toLowerCase()) ||
            item.adRegionId.toLowerCase().startsWith(value.toLowerCase())

          const includes =
            item.customerBusinessName.toLowerCase().includes(value.toLowerCase()) ||
            item.resPartitionName.toLowerCase().includes(value.toLowerCase()) ||
  ...
  return (
    <Fragment>
      <Breadcrumbs breadCrumbTitle='Respartition AD Details
' breadCrumbParent='Oracle Cloud
' breadCrumbActive='Respartition AD Details
' />
