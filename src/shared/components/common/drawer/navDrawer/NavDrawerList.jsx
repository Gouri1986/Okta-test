  ...
  return (
    <Tooltip title={sub.title} placement='top-end'>
      ...
      >
        <sub.Icon />
        {!collapsed && (
          <span className='ml-15 f-16 fw-600 lh-21 f-grey icon-name'>
            {sub.title}
          </span>
        )}
        <span>
            ...
            >
              {hoverSubMenu.map((menu) => (
                <li
                  ...
                  className='pr-15 pl-15 pt-10 pb-10 f-16 fw-600 lh-21 f-grey icon-name'
                >
                  {menu.title}
                </li>
              ))}
    ...
    <div className='drawer-list-container mt-100'>
      <ul>
        <li onClick={() => navigate("/iam")} className='cp p-12 flex-r-ac'>
          <IAMIcon />
          {!collapsed && (
            <span className='ml-20 f-16 fw-600 lh-21 f-grey icon-name'>
              IAM
            </span>
          )}
        </li>
      ...
      <div className='mt-20'>
        {encsDrawer().map((section) => (
          <div className='drawer-lists'>
            {!collapsed && (
              <span className={"fw-400 f-16 lh-21 f-darkgrey"}>
                {section.title}
              </span>
            )}
            <ul className='mt-10'>
