...
const SelectedFilter = ({
  ...
  const removeFilter = () => {
    const selectedFiltersFiltered = selectedColumn.filter(
      (e) => e.id !== column.id
    );
    ...
    >
      <p className='mr-7 f-12 fw-400 fc-secondary'>
        ...
        {column.value?.length > 50
          ? column.value.substr(0, 50) + "..."
          : column.value}
        {!column.value && (
          <FilterSearchValueInput
...
const FilterSearch = () => {
  const dispatch = useDispatch();
  const { tableContents, filteredTableContents } = useSelector(
  ...
  useEffect(() => {
    ...
    if (selectedColumnCopy.length > 0) {
      if (filteredTableContents.data?.length >= 1) {
        let filteredData = filteredTableContents?.data?.filter((e) => {
          console.log(activeColumn);
          if (
            e[selectedColumnCopy[selectedColumnCopy.length - 1]["id"]] ===
            selectedColumnCopy[selectedColumnCopy.length - 1]["value"]
          ) {
            return true;
          }
          return false;
  ...
  const onColumnValueChange = (value) => {
    ...
    if (value.length > 0 && filteredColumnNames?.length > 0) {
      setSuggestions(
        filteredColumnNames?.filter(
          (name) => !selectedColumn.find((e) => e.id === name.id)
        )
      );
    } else {
      setSuggestions([]);
  ...
  const onRowValueChange = (value, id) => {
    const filteredRowNames = data?.filter((e) =>
  ...
  const onSuggestionClick = (item) => {
    ...
    if (activeColumn.length > 0) {
      ...
      const activeColumnInEdit = selectedcolumnArray.find(
        (e) => e.id === activeColumn
      );
