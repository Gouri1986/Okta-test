...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-res-partition-master`,
    {
      headers: {               
...
//--Delete method-
const removeData = (resPartitionName) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-res-partition-master`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { resPartitionName : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(resPartitionName, {DeletePassFunction}) 
}
//--Delete method End-
 ...
 const columns = [
  ...
  {
    name: 'Description',
    selector: 'resPartitionDescription',
    sortable: true,
    minWidth: '350px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate' style={{width:'320px'}}>{row.resPartitionDescription}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  onClick={() => removeData(row.resPartitionName)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>Resource Partition Master</CardTitle>
