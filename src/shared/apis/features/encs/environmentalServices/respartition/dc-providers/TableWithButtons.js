...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-dc-providers`,
    {
      headers: {               
...
//--Delete method-
const removeData = (dcProviderId) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-dc-providers`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { dcProviderId : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(dcProviderId, {DeletePassFunction}) 
}
//--Delete method End-
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>Resource Partition DC Providers</CardTitle>
