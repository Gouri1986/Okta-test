...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-res-partition-dc-hall`,
    {
      headers: {               
...
//--Delete method-
const removeData = (resPartitionDcHallId) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-res-partition-dc-hall`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { resPartitionDcHallId : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(resPartitionDcHallId, {DeletePassFunction}) 
}
//--Delete method End-
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  onClick={() => removeData(row.resPartitionDcHallId)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>Resource Partition DC Hall</CardTitle>
