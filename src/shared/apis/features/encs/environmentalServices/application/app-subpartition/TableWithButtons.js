...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-app-sub-partition-details`,
    {
      headers: {               
...
//--Delete method-
const removeData = (appSubPartitionId) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-app-sub-partition-details`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { appSubPartitionId : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(appSubPartitionId, {DeletePassFunction}) 
}
//--Delete method End-
 ...
 const columns = [
  {
    ...
    selector: 'subpartition_name',
    sortable: true,
    minWidth: '350px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate ml-1'>
          <span className='d-block font-weight-bold text-truncate'>{row.appSubPartitionName}</span>
        </div>
      </div>
    )
  },
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate' style={{width:'150px'}}>{row.appPartitionName}</span>
 ...
 
  {
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'  style={{width:'250px'}}>{row.appSubPartitionDescription}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  
              onClick={() => removeData(row.appSubPartitionId)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>Application Sub Partition Details</CardTitle>
