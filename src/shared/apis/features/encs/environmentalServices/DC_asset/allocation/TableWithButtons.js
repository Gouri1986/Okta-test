...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-dc-asset-allocation`,
    {
      headers: {               
...
//--Delete method-
const removeData = (dcAssetId) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-dc-asset-allocation`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { dcAssetId : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(dcAssetId, {DeletePassFunction}) 
}
//--Delete method End-
 ...
 const columns = [
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate' style={{width:'190px'}}>{row.dcAssetName}</span>
  ...
  {
    name: 'Allocation Owner Group',
    selector: 'assetAllocOwnerGroup',
    sortable: true,
    minWidth: '300px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate ml-1'>
          <span className='d-block font-weight-bold text-truncate'>{row.assetAllocOwnerGroup}</span>
        </div>
      </div>
    )
  },
  {
    name: 'Allocation Request Type ',
    selector: 'allocRequestType',
    sortable: true,
    minWidth: '250px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.allocRequestType}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
  ...
  {
    name: 'Ticket Number',
    selector: 'ticketNunber',
    sortable: true,
    minWidth: '190px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.ticketNunber}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
  ...
  {
    name: 'Allocated Date ',
    selector: 'allocatedDate',
    sortable: true,
    minWidth: '190px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.allocatedDate}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100'  onClick={() => removeData(row.dcAssetId)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4" style={{ color: 'white'}}>DC Asset Allocation</CardTitle>
