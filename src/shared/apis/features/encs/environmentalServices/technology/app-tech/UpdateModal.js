...
const UpdateModal = ({ open, handleUpdateModal, value, fetchData}) => {
 ...
 const [token, setToken] = useState(
  ...
  const handlePost = (evt) => {
    ...
    axios.put(`${process.env.REACT_APP_URL}/api/v1/encs-app-tech-master`, {
...
return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleUpdateModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'style={{color:'black'}}>Update Application Technology </h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1'>
      <FormGroup>
          <Label for='isctechcategoryname'style={{fontSize:'1rem'}}>Application Technology Name*</Label>
          <InputGroup>
        ...
        <FormGroup>
          <Label for='isctechcategorytags'style={{fontSize:'1rem'}}>Application Technology Tags</Label>
          <InputGroup>
            <InputGroupAddon addonType='prepend'>
              <InputGroupText>
                <Briefcase size={15} />
