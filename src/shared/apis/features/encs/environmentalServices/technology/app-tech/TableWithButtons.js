...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-app-tech-master`,
    {
      headers: {               
...
//--Delete method-
const removeData = (iscAppTechName) => {
  const DeletePassFunction = (data) => {
    const deletevar = data
    console.log("deletevar = ", deletevar)
    axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-app-tech-master`,
    {
        headers: {               
          "Content-Type": "application/json",
          "access-token": `${token}`
        },
        data : { iscAppTechName : deletevar }
      }
      ).then((response) => {
  ...
  handleConfirmText(iscAppTechName, {DeletePassFunction}) 
}
//--Delete method End-
 ...
 const columns = [
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.iscAppTechName}</span>
  ...
  {
    name: 'Application technology tags',
    selector: 'iscTechCategoryTags',
    sortable: true,
    minWidth: '300px',
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate '>
          <span className='d-block font-weight-bold text-truncate'>{row.iscAppTechTags}</span>
          {/* <small>{row.post}</small> */}
        </div>
      </div>
    )
    ...
    cell: row => (
      <div className='d-flex align-items-center'>
        <div className='user-info text-truncate ml-1'>
          <span className='d-block font-weight-bold text-truncate'  style={{width:'250px'}}>{row.iscAppTechDescription}</span>
        </div>
      </div>
    )
  },
    ...
    cell: row => {
      return (
        <div className='d-flex'>
              ...
              <div tag='a'  className='w-100' 
               onClick={() => removeData(row.iscAppTechName)}>
                <Trash size={15} />
              </div>
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4"style={{ color: 'white'}}>Application Technology Master</CardTitle>
