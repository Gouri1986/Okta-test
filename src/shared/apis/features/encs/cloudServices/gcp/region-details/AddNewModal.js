...
import { X, Layers, Database, Command, Plus } from 'react-feather'
import {
  Button,
  Modal,
  ModalHeader,
...
const AddNewModal = ({ open, handleModal, fetchData }) => {
  ...
  const changeSelectOptionHandlercloudName = (event) => {
  ...
  const handlePost = (evt) => {
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-gcp-region-details`, {
      regionId: regionid,
      cloudName: cloudname,
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title' style={{ color: 'black' }}>New GCP Region Details</h5>
      </ModalHeader>
      <ModalBody className='flex-grow-1' onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input id='RegionID'
              name='RegionID'
              innerRef={register({ required: true })} invalid={errors.RegionID && true} placeholder='' onChange={(e) => setregionId(e.target.value)} />
        ...
        <FormGroup>
          ...
          <InputGroup>
            ...
            <Input type='select' name='Cloudname' id='Cloudname' onChange={changeSelectOptionHandlercloudName}>
              <option hidden selected disabled>Choose Cloud Name</option>
        ...
        <FormGroup>
          ...
          <InputGroup>
            ...
            {errors && errors.RegionLocation && <FormFeedback>Region Location must be at least 10 characters</FormFeedback>}
          </InputGroup>
        </FormGroup>
