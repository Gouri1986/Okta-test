...
const AddNewModal = ({ open, handleModal, fetchData}) => {
  ...
  const handlePost = (evt) => {
    axios.post(`${process.env.REACT_APP_URL}/api/v1/encs-gcp-res-partition-az`, {
      azId : idAz,
      azRegionId : regionId,
...
useEffect(() => { 
  // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/list-gcpAzIds`
  )
  .then((response) => {
    // Handle success.
    console.log("Connection established.Data is fetching!")
...
//--Fetching Dropdown ends
  ...
  const SignupSchema = yup.object().shape({
    gcpResourcePartition: yup.string().min(10).required()
  })
  ...
  return (
    ...
    >
      <ModalHeader className='mb-3' toggle={handleModal} close={CloseBtn} tag='div'>
        <h5 className='modal-title'style={{color:'black'}}>New GCP Resource Partition AZ</h5>
      </ModalHeader>
      ...
      <FormGroup>
        ...
        <InputGroupAddon addonType='prepend'>
          ...
          <Input type='select' name='select' id='Customer' onChange={CustomerDropDown}>
            <option disabled hidden selected>Choose cutomer</option>
            {data.map(item => (
              <option
        ...
        <InputGroupAddon addonType='prepend'>
          ...
          <Input type='select' name='select' id='Partition-Name' onChange={ChnagePartition}>
        ...
        <InputGroupAddon addonType='prepend'>
          ...
          <Input type='select' name='select' id='AZ-ID' onChange={ChangeAzid}>
        ...
        <FormGroup>
        <Label for='Region-Id' style={{fontSize:'1rem'}}>Region ID</Label>
        <InputGroup>
        <InputGroupAddon addonType='prepend'>
              <InputGroupText>
              <Layers size={15}/>
          ...
          <Input type='select' name='select' id='select-basic' onChange={ChangeRegionId}>
          <option disabled hidden selected>Choose Region ID</option>
            {datathree.map(item => (
              <option
