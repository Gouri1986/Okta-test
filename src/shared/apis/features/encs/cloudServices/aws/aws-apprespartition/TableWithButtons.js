...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-aws-app-res-partition-az`,
      {
        headers: {
  ...
  const removeData = (awsAppResPartitionAzId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-aws-app-res-partition-az`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { awsAppResPartitionAzId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(awsAppResPartitionAzId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End----------------------------------------------------------------------
  ...
  const columns = [
    ...
    {
      name: 'AWS Resource Partition AZ',
      selector: 'awsResPartitionAzId',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '200px' }}>{row.awsResPartitionAzId}</span>
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.awsAppResPartitionAzId)}>
              <Trash size={15} />
            </div>
  ...
  const handleFilter = (e) => {
    ...
    if (value.length) {
      updatedData = data.filter((item) => {
        try {
          const startsWith =
            item.awsResPartitionAzId.toLowerCase().startsWith(value.toLowerCase()) ||
            item.appSubPartitionName.toLowerCase().startsWith(value.toLowerCase())

          const includes =
            item.awsResPartitionAzId.toLowerCase().includes(value.toLowerCase()) ||
