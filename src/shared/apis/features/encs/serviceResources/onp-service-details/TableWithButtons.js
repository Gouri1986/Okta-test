...
const DataTableWithButtons = () => {
  ...
  useEffect(() => {
    // if (isLoaded === false) {
    axios.get(`${process.env.REACT_APP_URL}/api/v1/encs-onp-service-master-details`,
      {
        headers: {
  ...
  //-----------------------------------------Delete method-------------------
  const removeData = (onpServiceMasterId) => {
    const DeletePassFunction = (data) => {
      const deletevar = data
      console.log("deletevar = ", deletevar)
      axios.delete(`${process.env.REACT_APP_URL}/api/v1/encs-onp-service-master-details`,
        {
          headers: {
            "Content-Type": "application/json",
            "access-token": `${token}`
          },
          data: { onpServiceMasterId: deletevar }
        }
      ).then((response) => {
    ...
    handleConfirmText(onpServiceMasterId, { DeletePassFunction })
  }
  //-----------------------------------------Delete method End-----------------------
  ...
  const columns = [
    ...
    {
      name: 'ONP Service Name',
      selector: 'onpServiceName',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.onpServiceName}</span>
    ...
    {
      name: 'Description',
      selector: 'onpServiceMasterDescription',
      sortable: true,
      minWidth: '300px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '250px' }}>{row.onpServiceMasterDescription}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '150px' }}>{row.customerName}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    ...
    {
      name: 'ONP Service Type',
      selector: 'onpServiceType',
      sortable: true,
      minWidth: '190px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.onpServiceType}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    ...
    {
      name: 'ONP Technology Type',
      selector: 'onpTechnologyType',
      sortable: true,
      minWidth: '250px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate ml-1'>
            <span className='d-block font-weight-bold text-truncate'>{row.onpTechnologyType}</span>
          </div>
        </div>
      )
    },
    {
      name: 'OS Technology Master',
      selector: 'encsOsTechMaster',
      sortable: true,
      minWidth: '240px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.encsOsTechMaster}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    ...
    {
      name: 'Data Technology Master',
      selector: 'encsDataTechMaster',
      sortable: true,
      minWidth: '260px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.encsDataTechMaster}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    ...
    {
      name: 'Application Technology Master',
      selector: 'encsAppTechMaster',
      sortable: true,
      minWidth: '320px',
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate'>{row.encsAppTechMaster}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
    },
    {
      ...
      cell: row => (
        <div className='d-flex align-items-center'>
          <div className='user-info text-truncate '>
            <span className='d-block font-weight-bold text-truncate' style={{ width: '300px' }}>{row.encsTags}</span>
            {/* <small>{row.post}</small> */}
          </div>
        </div>
      )
      ...
      cell: row => {
        return (
          <div className='d-flex' style={{ cursor: 'pointer' }}>
            ...
            <div tag='a' className='w-100' onClick={() => removeData(row.onpServiceMasterId)}>
              <Trash size={15} />
            </div>
          </div>
        )
  ...
  return (
    <Fragment>
      <Breadcrumbs
        breadCrumbTitle='ONP Service Master'
        breadCrumbParent='Services & Resources'
        breadCrumbActive='ONP Service Master'
      />
